...
import { helloWorldDocument, createNode, getNode, getSlot, insertNode, removeNode } from "../../js/model.js";
...
function Editor() {
  ...
  useEffect(() => {
    ...
    for (const node of flow.nodes) node.selected = node.data.astId === selectedId;
  ...
  // Reflect selection highlight without a full relayout.
  useEffect(() => {
  ...
  // Prefill default input from the compiler unless the user has edited it.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (data.ok) setStatusMsg(`Compiled ${data.application || "program"}`, "ok");
